...
import {checkIsWorkable} from './is-workable';
...
export class FlashbotBroadcastor {
  ...
  async tryToWorkOnFlashbots(jobContract: Contract, workMethod: string, methodArguments: Array<number | string>, isWorkable: boolean) {
    if (isWorkable != (await checkIsWorkable(jobContract, methodArguments[0], methodArguments[1]))) return;
